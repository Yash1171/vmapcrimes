import Navbar from "../../components/navbar/Navbar"
import Sidebar from "../../components/sidebar/Sidebar"
import "./newroles.scss"
import DriveFolderUploadIcon from '@mui/icons-material/DriveFolderUpload';
import { useState, useContext, useEffect } from 'react'
import UserContext from "../../context/userContext";
import { useNavigate } from "react-router-dom";

function Newroles() {
  const { isAuthenticated, verifyAccess, verifyAuth } = useContext(UserContext)
  const [data, setData] = useState({});
  const [updated, setUpdated] = useState(0)
  const [message, setMessage] = useState(false)
  // TODO: fetch perms from backend if user has appropriate permissions
  const { perms } = useContext(UserContext)
  const nav = useNavigate()

  useEffect(() => {

    if (!isAuthenticated) {
      console.log("Not authenticated")
      nav("/login", { replace: true })

    }

  }, [])

  const onBtnClick = async (e) => {
    if (isAuthenticated) {

      const options = {

        method: 'POST',
        credentials: 'include',
        body: JSON.stringify(data),
        headers: { "Content-Type": "application/json" }

      }

      var resp = await fetch("http://localhost:5001/api/roles/addRole", options)
      var respJSON = await resp.json()
      verifyAuth(resp, respJSON)
      if(respJSON.status==="success"){
        setUpdated(1)
        
    }else { setUpdated(2) }
    setMessage(respJSON.message?respJSON.message:respJSON.errors.map((item)=>{return item.msg}))
    

    }
  }

  const handleChange = (event) => {
    var newData = data
    if (event.target.name === "name") {
      data["role"] = event.target.value
    }
    if (event.target.checked) {
      switch (event.target.name) {
        case "actionPerms":
          newData["action_perms"] ? newData["action_perms"].push(event.target.value) : newData["action_perms"] = [event.target.value];
          break
        case "readPerms":
          newData["read_perms"] ? newData["read_perms"].push(event.target.value) : newData["read_perms"] = [event.target.value];
          break
        default:
          break;
      }
    } else {

      switch (event.target.name) {
        case "actionPerms":
          newData["action_perms"] = newData["action_perms"].filter((i) => { return i !== event.target.value });
          break
        case "readPerms":
          newData["read_perms"] = newData["read_perms"].filter((i) => { return i !== event.target.value });
          break
        default:
          break;
      }

    }

    setData(newData);
    console.log("Data is now " + JSON.stringify(data))
  };
  return (
    <div className="newroles">
      <Sidebar />
      <div className="newrolescontainer">
        <Navbar />
        {verifyAccess({ ACTION_PERMS: ["ADD_NEW_ROLE"] }) ?
          <>

            <div className="top">
              <h1>Add New Roles</h1>
            </div>
            <div className="bottom">
              <div className="left">
                <img src="data:image/png;base64,"
                  alt="profile"
                />
              </div>
              <div className="right">
                <form>
                  <div className="formInput">
                    <label htmlFor="file">
                      Role Icon: <DriveFolderUploadIcon className="icon" />
                    </label>
                    <input type="file" id="file" style={{ display: "none" }} />
                  </div>
                  <div className="formInput">
                    <label>Name:</label>
                    <input type="text" name="name" onChange={handleChange} placeholder="Constable" />
                  </div>
                  <div className="formInput">
                    <label>Action Permissions:</label>

                    {perms.ACTION_ROLES.map(item => { return <label htmlFor=""><input type="checkbox" onChange={handleChange} name="actionPerms" value={item} />{item}</label> })}


                  </div>
                  <div className="formInput" >
                    <label>Read Permissions:</label>
                    {perms.READ_ROLES.map(item => { return <label htmlFor=""><input type="checkbox" onChange={handleChange} name="readPerms" value={item} />{item}</label> })}

                    {/* <label htmlFor=""><input type="checkbox" onChange={handleChange} name="readPerms" id="date" value="DATE_OF_CRIME"/>Can view date of crime</label>
                <label htmlFor=""><input type="checkbox" onChange={handleChange} name="readPerms" id="weapons" value="CRIME_USED_WEAPONS"/>Can view weapons used</label>
                <label htmlFor=""><input type="checkbox" onChange={handleChange} name="readPerms" id="evidence" value="CRIME_EVIDENCE"/>Can view evidence</label>
                <label htmlFor=""><input type="checkbox" onChange={handleChange} name="readPerms" id="suspects" value="CRIME_SUSPECTS"/>Can view name of suspects</label>
                  */}

                  </div>





                </form>
                <button onClick={onBtnClick}>ADD</button>
                {updated? updated === 1 ? <p style={{color:"green",display:"inline !important"}}>{message}</p>:<span style={{color:"red"}}>{Array.isArray(message)?message.map((item)=>{return <><br />{item}</>}):message}</span>:null}

              </div>
            </div>
          </> : !isAuthenticated ? <h3>Please Log in to Continue</h3> : <h3>You don't have permissions to add new roles</h3>}
      </div>
    </div>
  )
}

export default Newroles
